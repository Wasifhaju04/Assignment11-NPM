...
def Create(request):
    if request.method == "POST":
        ...
        if(request.user is None):
            messages.add_message(request, messages.ERROR, 'YOU ARE NOT LOGGED IN. PLEASE LOGIN AND COME BACK !!! ')
        ...
        messages.add_message(request, messages.ERROR, 'DEAR USER, YOU ARE NOT ALLOWED HERE !!!')
        return redirect("/project/")
...
def Delete(request,id):
    ...
    if((project.submitted_by.id == request.user.id) or request.user.is_superuser):
        ...
        messages.add_message(request, messages.ERROR, 'DEAR USER, YOU CANNOT DELETE THIS PROJECT')
        return redirect("/project/")    
    
    messages.add_message(request, messages.SUCCESS, 'PROJECT DELETED SUCCESFULLY ')
    return redirect("/project/")
...
def Edit(request,id):
    if request.method == "POST":
        ...
        # validations
        if(not(len(enroll_no)==10)):
            messages.add_message(request, messages.ERROR, 'ENTER ENROLLMENT NUMBER IS INVALID')
            return redirect("/project/create/")
        if( not (validators.url(git_link))):
            messages.add_message(request, messages.ERROR, 'ENTER LINK IS INVALID')
            return redirect("/project/create/")
        else:
            git_link = formaturl(git_link)
        if( len(live_link)<0 and not (validators.url(live_link))):
        ...
        try:
            ...
            project.save()
            messages.add_message(request, messages.SUCCESS,"DEAR USER, PROJECT EDIT SUCCESSFULLY..")
            return redirect("/project/")
        except Exception as e:
                messages.add_message(request, messages.ERROR, e)
                return redirect("/project/")
